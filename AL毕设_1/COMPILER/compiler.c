...
static int findUpvalue(CompileUnit* cu, const char* name, uint32_t length) {
   ...
   if (directOuterLocalIndex != -1) {
      cu->enclosingUnit->localVars[directOuterLocalIndex].isUpvalue = YES;
      return addUpvalue(cu, YES, (uint32_t)directOuterLocalIndex);
   }

   //向外层递归查找
   int directOuterUpvalueIndex = findUpvalue(cu->enclosingUnit, name, length);
   if (directOuterUpvalueIndex != -1) {
      return addUpvalue(cu, NO, (uint32_t)directOuterUpvalueIndex);
   }
...
static void emitGetterMethodCall(CompileUnit* cu, Signature* sign, OpCode opCode) {
  ...
  //下面调用的processArgList是把实参入栈,供方法使用
   ...
   if (matchToken(cu->curParser, TOKEN_LEFT_BRACE)) {
      ...
      newSign.type = SIGN_METHOD;
      CompileUnit fnCU;
      initCompileUnit(cu->curParser, &fnCU, cu, NO);
...
static void condition(CompileUnit* cu, bool canAssign UNUSED) {
   //若condition为NO, if跳转到NO分支的起始地址,为该地址设置占位符
   uint32_t NOBranchStart = emitInstrWithPlaceholder(cu, OPCODE_JUMP_IF_FALSE);

   //编译YES分支
   expression(cu, BP_LOWEST);

   consumeCurToken(cu->curParser, TOKEN_COLON, "expect ':' after YES branch!");
   
   //执行完YES分支后需要跳过NO分支
   uint32_t NOBranchEnd = emitInstrWithPlaceholder(cu, OPCODE_JUMP);
   
   //编译YES分支已经结束.此时知道了YES分支的结束地址,
   //编译NO分支之前须先回填NOBranchStart
   patchPlaceholder(cu, NOBranchStart);

   //编译NO分支.
   expression(cu, BP_LOWEST);

   //知道了NO分支的结束地址,回填NOBranchEnd
   patchPlaceholder(cu, NOBranchEnd); 
}
...
//   2 解析内嵌表达式,如"b+c",将其结果添加到list
   do {
      ...
      //将结果添加到list
      emitCall(cu, 1, "addCore_(_)", 11);
   } while (matchToken(cu->curParser, TOKEN_INTERPILATION)); // 处理下一个内嵌表达式,如"a %(b+c) d %(e) f "中的" d %(e)"
...
static void compileIfStatment(CompileUnit* cu) {
   ...
   //若条件为假, if跳转到NO分支的起始地址,现为该地址设置占位符
   uint32_t NOBranchStart = 
   ...
   if (matchToken(cu->curParser, TOKEN_ELSE)) {
      ...
      //进入else分支编译之前,先回填NOBranchStart
      patchPlaceholder(cu, NOBranchStart);

      //编译else分支
      compileStatment(cu);

      //此时知道了NO块的结束地址,回填NOBranchEnd
      patchPlaceholder(cu, NOBranchEnd);

   } else {   //若不包括else块
      //此时NOBranchStart就是件为假时,需要跳过整个YES分支的目标地址
      patchPlaceholder(cu, NOBranchStart);
   }
}
...
static void emitCreateInstance(CompileUnit* cu,
   ...
   initCompileUnit(cu->curParser, &methodCU, cu, YES);
...
static void compileMethod(CompileUnit* cu, Variable classVar, bool isStatic) {
   ...
   if (sign.type == SIGN_CONSTRUCT) {
      ...
      //构造函数是静态方法,即类方法
      defineMethod(cu, classVar, YES, constructorIndex);
   }
}
...
static void compileClassBody(CompileUnit* cu, Variable classVar) {
   ...
   if (matchToken(cu->curParser, TOKEN_STATIC)) {
	 ...
	 compileVarDefinition(cu, YES);
      } else {   //处理静态方法,"static methodName"
	 compileMethod(cu, classVar, YES);
      }

   } else if (matchToken(cu->curParser, TOKEN_VAR)) {   //实例域
      compileVarDefinition(cu, NO);
   } else {  //类的方法
      compileMethod(cu, classVar, NO);
   }
}
...
static void compileFunctionDefinition(CompileUnit* cu) {
  ...
  //       函数名(实参)
   ...
   uint32_t fnNameIndex = declareVariable(cu, fnName, (uint32_t)strlen(fnName));
...
ObjFn* compileModule(VM* vm, ObjModule* objModule, const char* moduleCode) {
   ...
   initCompileUnit(&parser, &moduleCU, NULL, NO);
